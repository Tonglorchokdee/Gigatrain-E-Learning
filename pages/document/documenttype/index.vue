  ...
  <v-row justify="center" >
    <v-col cols="12" sm="12" md="12">
      ...
      <div class="border-bottom mb-6">
        <v-btn  to="documenttype/create" color="primary" class="btn-primary" large dark >
          <v-icon>mdi-plus</v-icon>  เพิ่มประเภทเอกสาร
        </v-btn>
      </div>

      <v-card>
        <v-card-title class="headline">จัดการประเภทเอกสาร</v-card-title>
        <v-card-text>
          <v-simple-table>
                ...
                <tr v-for="(item, i) in list" :key="item.name">
                  ...
                  <td class="text-center">
                    <v-btn
                      @click="
                        $router.push({
                          name: 'document-documenttype-edit',
                          query: { id: item.id },
                        })
                      "
                      color="primary"
                    ...
                    <v-btn
                      ...
                      @click="
                        $router.push({
                          name: 'document-documenttype-create',
...
export default {
  ...
  methods: {
    loadData: async function () {
      ...
      this.list = await documenttypeService.admin();
    ...
    deleteData: async function (id) {
      try {
        ...
        if (await documenttypeService.delete(params)) {
          this.loadData();
        }
      } catch (error) {
        console.error(error);
    ...
    checkParent: async function (id) {
      ...
      return await documenttypeService.parentlang(params);
    },
  },
};
</script>
