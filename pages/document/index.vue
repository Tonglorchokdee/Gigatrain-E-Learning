  ...
  <v-row justify="center" >
    <v-col cols="12" sm="12" md="12">
      ...
      <div class="border-bottom mb-6">
        <v-btn  to="document/create" color="primary" class="btn-primary" large dark >
          <v-icon>mdi-plus</v-icon>  เพิ่มเอกสาร
        </v-btn>
      </div>

      <v-card>
        <v-card-title class="headline">จัดการเอกสาร</v-card-title>
        <v-card-text>
          <v-simple-table>
            <template v-slot:default>
              <thead>
                <tr>
                  ...
                  <th class="text-center">ประเภทไฟล์เอกสาร</th>
...
export default {
  ...
  methods: {
    loadData: async function () {
      ...
      this.list = await documentService.admin();
    ...
    deleteData: async function (id) {
      try {
        ...
        if (await documentService.delete(params)) {
          this.loadData();
        }
      } catch (error) {
        console.error(error);
    ...
    checkParent: async function (id) {
      ...
      return await documentService.parentlang(params);
    },
  },
};
</script>
