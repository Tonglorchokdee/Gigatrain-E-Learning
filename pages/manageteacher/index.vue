<template>
  <v-row justify="center">
    <v-col cols="12" sm="12" md="12">
      <div class="border-bottom mb-6">
        <v-btn
          to="manageteacher/create"
          color="primary"
          class="btn-primary"
          large
          dark
          v-bind="attrs"
          v-on="on"
        >
          <v-icon>mdi-plus</v-icon> เพิ่มอาจารย์ผู้อบรม
        </v-btn>
      </div>

      <v-card>
        <v-card-title class="headline">จัดการอาจารย์ผู้อบรม</v-card-title>
        <v-card-text>
          <v-simple-table>
            <template v-slot:default>
              <thead>
                <tr>
                  <th class="text-center">ลำดับ</th>
                  <th class="text-center">ชื่อ</th>
                  <th class="text-center">นามสกุล</th>
                  <th class="text-center">อีเมล</th>
                  <th class="text-center">เบอร์ติดต่อ</th>
                  <th class="text-center">รูปภาพอาจารย์</th>
                  <th class="text-center">ภาษา</th>
                  <th class="text-center">จัดการ</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(item, i) in list" :key="item.teacher_id">
                  <td class="text-center">{{ i + 1 }}</td>
                  <td>{{ item.teacher_fname }}</td>
                  <td>{{ item.teacher_lname }}</td>
                  <td>{{ item.teacher_email }}</td>
                  <td>{{ item.teacher_tel }}</td>
                  <td>
                    <v-img
                      :lazy-src="item.image"
                      max-height="150"
                      max-width="250"
                      :src="item.image"
                    ></v-img>
                  </td>
                  <td class="text-center">
                    <v-btn
                      @click="
                        $router.push({
                          name: 'manageteacher-edit',
                          query: { id: item.teacher_id },
                        })
                      "
                      color="primary"
                      class="mt-1 mb-1"
                      small
                      >EN (แก้ไข)</v-btn
                    >

                    <v-btn
                      v-if="item.parent_id == 0"
                      @click="
                        $router.push({
                          name: 'manageteacher-create',
                          query: { parent_id: item.teacher_id, lang: 'th' },
                        })
                      "
                      color="primary"
                      class="mt-1 mb-1"
                      small
                      >TH (เพิ่ม)</v-btn
                    >

                    <v-btn
                      v-if="item.parent_id != 0"
                      @click="
                        $router.push({
                          name: 'manageteacher-edit',
                          query: { id: item.parent_id, lang: 'th' },
                        })
                      "
                      color="primary"
                      class="mt-1 mb-1"
                      small
                      >TH (แก้ไข)</v-btn
                    >
                  </td>
                  <td class="text-center">
                    <v-btn
                      @click="deleteData(item.teacher_id)"
                      color="error"
                      small
                    >
                      <v-icon>mdi-delete</v-icon>
                    </v-btn>
                  </td>
                </tr>
              </tbody>
            </template>
          </v-simple-table>
        </v-card-text>
      </v-card>
    </v-col>
  </v-row>
</template>

<script>
import Helper from "@/components/Helper.vue";
import { uploadService } from "@/plugins/api";
export default {
  auth: true,
  name: "article",
  layout: "dashboard",
  data: function () {
    return {
      loading: true,

      list: [],
    };
  },
  mounted() {
    this.loadData();
  },
  methods: {
    loadData: async function () {
      this.loading = true;
      this.list = [];
      // TODO: กำหนด Request ที่ต้องการ แล้ว remap data ให้ตรง
      this.list = await this.admin();
      this.list.forEach(async (item) => {
        item.date = await Helper.methods.datetimeShow(item.date);
        item.fixdate = await Helper.methods.datetimeShow(item.fixdate);
        if (item.image != "" && item.image != null) {
          item.image = await uploadService.downloadFile(
            item.image,
            "image/png"
          );
        }
        item.parent_id = await this.checkParent(item.teacher_id);
      });
      this.loading = false;
    },
    deleteData: async function (id) {
      try {
        const params = {
          teacher_id: id,
          deleted_by: 0,
        };
        var parent_id = await this.checkParent(id);
        const paramsParent = {
          teacher_id: parent_id,
          deleted_by: 0,
        };
        if ((await this.delete(paramsParent)) && (await this.delete(params))) {
          this.loadData();
        }
      } catch (error) {
        console.error(error);
      }
    },
    checkParent: async function (id) {
      const params = {
        teacher_id: id,
      };
      return await this.parentLang(params);
    },
    admin: async function () {
      return this.$axios
        .$request({
          url: "/course/teacher",
          method: "GET",
        })
        .then((response) => {
          if (response.status == 200) {
            this.list = [];
            response.teachers
              .filter((x) => x.parent_id == 0)
              .forEach((element) => {
                this.list.push({
                  teacher_id: element.teacher_id,
                  teacher_fname: element.teacher_fname,
                  teacher_lname: element.teacher_lname,
                  teacher_email: element.teacher_email,
                  teacher_tel: element.teacher_tel,
                  image: element.image,
                  date: element.created_date,
                  fixdate: element.updated_date,
                  parent_id: 0,
                });
              });
            return this.list;
          }
        })
        .catch((err) => console.error(err));
    },
    parentLang: async function (params) {
      var id = 0;
      return this.$axios
        .$request({
          url: "/course/teacher/parentLang/" + params.teacher_id,
          method: "GET",
        })
        .then((response) => {
          if (response.ParentLang.length > 0) {
            id = response.ParentLang[0].teacher_id;
          }
          return id;
        })
        .catch((err) => console.error(err));
    },
    delete: async function (params) {
      return this.$axios
        .$request({
          url: "/course/teacher/" + params.teacher_id,
          method: "DELETE",
        })
        .then((response) => {
          if (response) {
            return true;
          } else {
            return false;
          }
        })
        .catch((err) => console.error(err));
    },
  },
};
</script>

